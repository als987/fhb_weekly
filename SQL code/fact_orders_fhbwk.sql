create
or replace view beam_prod.finance_azlab_prod.fact_orders_fhbwk AS
select
    cal.fiscalWeek,
    ...
    CASE
        ...
        ELSE 'F'
    END AS fp_rp_ind,
    SUM(ord.grossSalesInGC) AS gsriv,
    SUM(ord.grossSalesInGC - ord.vatInGC) AS gsrev,
    SUM(ord.netValueInGC) as sriv,
    SUM(ord.netValueInGC - ord.vatInGC) as srev,
    SUM(ord.vatInGC) as vat,
    SUM(realisedMarkdownInclVat) AS rmiv,
    SUM(realisedMarkdownExclVat) AS rmev,
    SUM(ord.staffDiscountInGC) AS stfdisc,
    SUM(ord.otherDiscountInGC) AS othdisc,
    SUM(ord.netValueInGC - ord.vatInGC - ord.costInGC) as gsm,
    SUM(
        ord.grossSalesInGC - ord.vatInGC + ord.realisedMarkdownExclVat - ord.costInGC
    ) as bimev_num,
    SUM(
        ord.grossSalesInGC - ord.vatInGC + ord.realisedMarkdownExclVat
    ) as bimev_denom
from
    beam_prod.analytics_cnh_prod.factsales_saz_tbl ord
...
where
    ...
    AND (
        (
            dataProvider IN ("Retail")
            AND distributionChannel IN ("01", "05")
        )
        OR (
            dataProvider = "Weborder"
            AND distributionChannel IN ("03")
        )
    )
group by
    cal.fiscalWeek,
